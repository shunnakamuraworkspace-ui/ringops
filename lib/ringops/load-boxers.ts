...
const divisions: Record<string, string> = {
  atom: "アトム級", atomweight: "アトム級", mini_fly: "ミニフライ級", mini_flyweight: "ミニフライ級",
  minimum: "ミニマム級", minimumweight: "ミニマム級", light_fly: "ライトフライ級", light_flyweight: "ライトフライ級",
  fly: "フライ級", flyweight: "フライ級", super_fly: "スーパーフライ級", super_flyweight: "スーパーフライ級",
  bantam: "バンタム級", bantamweight: "バンタム級", super_bantam: "スーパーバンタム級", super_bantamweight: "スーパーバンタム級",
  feather: "フェザー級", featherweight: "フェザー級", super_feather: "スーパーフェザー級", super_featherweight: "スーパーフェザー級",
  light: "ライト級", lightweight: "ライト級", super_light: "スーパーライト級", super_lightweight: "スーパーライト級",
  welter: "ウェルター級", welterweight: "ウェルター級", super_welter: "スーパーウェルター級", super_welterweight: "スーパーウェルター級",
  middle: "ミドル級", middleweight: "ミドル級", super_middle: "スーパーミドル級", super_middleweight: "スーパーミドル級",
  light_heavy: "ライトヘビー級", light_heavyweight: "ライトヘビー級", cruiser: "クルーザー級", cruiserweight: "クルーザー級",
  heavy: "ヘビー級", heavyweight: "ヘビー級",
};
...
const prefectures: Record<string, string> = {
  ...
  "21": "岐阜県", "22": "静岡県", "23": "愛知県", "24": "三重県", "25": "滋賀県", "26": "京都府", "27": "大阪府", "28": "兵庫県",
...
function reviewResult(): LoadResult {
  return {
...
export async function loadBoxers(): Promise<LoadResult> {
  if (!isSupabaseConfigured || await shouldUseReviewMode()) return reviewResult();
  ...
  if (user) {
    ...
    if (membershipError) {
      return { boxers: [], databaseConnected: true, industryMode: false, reviewMode: false, loadError: "業界アカウント情報を読み込めませんでした。" };
    }
  ...
  if (boxerError) {
    return { boxers: [], databaseConnected: true, industryMode, reviewMode: false, loadError: "選手データを読み込めませんでした。時間をおいて再度お試しください。" };
  }
  if (!boxerRows?.length) return { boxers: [], databaseConnected: true, industryMode, reviewMode: false, loadError: null };
  ...
  if (organizationResult.error || rankingResult.error || statusResult.error) {
    return { boxers: [], databaseConnected: true, industryMode, reviewMode: false, loadError: "選手関連データの読み込みに失敗しました。管理者へお問い合わせください。" };
  }
  ...
  const boxers: BoxerPreview[] = boxerRows.map((row) => {
    ...
    const domesticOrInternational: BoxerPreview["domesticOrInternational"] = row.domestic_or_international === "international" ? "海外" : "国内";
    const competitionCategory: BoxerPreview["competitionCategory"] = row.competition_category === "women" ? "女子" : row.competition_category === "men" ? "男子" : undefined;
  ...
  return { boxers, databaseConnected: true, industryMode, reviewMode: false, loadError: null };
}
