...
const options=["募集中","相談中","交渉中","ジム確認待ち","内定","決定","中止"];

export function CaseBoard({ databaseConnected, industryMode }: { databaseConnected:boolean; industryMode:boolean }) {
  const [items,setItems]=useState<CaseItem[]>(databaseConnected?[]:defaults); const [loading,setLoading]=useState(databaseConnected); const [error,setError]=useState("");
    ...
    (async()=>{
      try{
        ...
        const {data:cases,error:caseError}=await supabase.schema("ringops").from("matchmaking_cases")
          .select("id,proposed_event_name,event_date,contract_weight_kg,rounds,status,boxer_b_id,created_at")
          .order("updated_at",{ascending:false});
        if(caseError)throw caseError;
        const boxerIds=[...new Set((cases??[]).map((item:any)=>item.boxer_b_id).filter(Boolean))];
        const {data:boxers}=boxerIds.length?await supabase.schema("ringops").from("boxers").select("id,name,organization_id").in("id",boxerIds):{data:[]};
  ...
  return <div className="border-y-2 border-slate-950 bg-white">{error&&<div className="border-b border-rose-200 bg-rose-50 px-5 py-3 text-xs font-bold text-rose-800">{error}</div>}<div className="hidden grid-cols-[1.35fr_1.35fr_1fr_.7fr_1fr] gap-4 border-b border-slate-200 bg-slate-50 px-5 py-3 text-[10px] font-black text-slate-500 md:grid"><span>選手 / ジム</span><span>興行 / 日程</span><span>条件</span><span>R</span><span>進捗</span></div>{items.map(i=><article className="grid gap-3 border-b border-slate-200 px-5 py-5 last:border-0 md:grid-cols-[1.35fr_1.35fr_1fr_.7fr_1fr] md:items-center" key={i.id}><div><b>{i.boxer}</b><p className="mt-1 text-xs text-slate-500">{i.gym}</p></div><div><b className="text-sm">{i.event||"興行未設定"}</b><p className="mt-1 text-xs text-slate-500">{i.date||"日程未設定"}</p></div><b className="text-sm">{i.weight||"—"}</b><b>{i.rounds?`${i.rounds}R`:"—"}</b><select className="h-10 border border-slate-300 px-2 text-sm font-bold" value={i.status} onChange={e=>update(i.id,e.target.value)}>{options.map(v=><option key={v}>{v}</option>)}</select></article>)}{!items.length&&<div className="px-5 py-12 text-center text-sm font-bold text-slate-500">現在の案件はありません。</div>}</div>;
}
