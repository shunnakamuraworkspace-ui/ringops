...
const demo:EventItem[]=[{
  ...
  status:"matching",
  bouts:[
    {id:"b1",eventId:"demo-1",boxerA:"山田 直樹",boxerB:"対戦相手募集中",rounds:6,weight:"55.0kg",status:"募集中"},
    {id:"b2",eventId:"demo-1",boxerA:"佐藤 海斗",boxerB:"田中 健",rounds:6,weight:"55.0kg",status:"決定"},
  ],
}];
...
export function EventBoard({databaseConnected,industryMode}:{databaseConnected:boolean;industryMode:boolean}){
    ...
    (async()=>{
      try{
        ...
        const [{data:orgs,error:orgError},{data:boutRows,error:boutError}]=await Promise.all([
          ...
          eventIds.length
            ? supabase.schema("ringops").from("bouts").select("id,event_id,boxer_a_id,boxer_b_id,contract_weight_kg,scheduled_rounds,matchmaking_status").in("event_id",eventIds).order("created_at")
            : Promise.resolve({data:[],error:null}),
        ]);
        ...
        const mapped:EventItem[]=(eventRows??[]).map((event)=>({
          ...
          promoter:orgMap.get(event.promoter_organization_id)??"興行主",
          status:event.status,
          bouts:(boutRows??[]).filter((bout)=>bout.event_id===event.id).map((bout)=>({
            id:bout.id,
            eventId:event.id,
            boxerA:boxerMap.get(bout.boxer_a_id)??"選手A未定",
            boxerB:boxerMap.get(bout.boxer_b_id)??"対戦相手募集中",
            rounds:bout.scheduled_rounds,
            weight:bout.contract_weight_kg!=null?`${bout.contract_weight_kg}kg`:"—",
            status:boutStatusLabels[bout.matchmaking_status]??"募集中",
          })),
        }));
  ...
  function persistReview(next:EventItem[]){
    setEvents(next);
    localStorage.setItem("ringops_events",JSON.stringify(next));
  }
  ...
  async function createEvent(e:React.FormEvent){
    ...
    try{
      ...
      const {data:memberships,error:membershipError}=await supabase.schema("ringops").from("organization_memberships")
        ...
        .eq("status","active")
        .in("role",["owner","admin","matchmaker"]);
      ...
      const {data:created,error:insertError}=await supabase.schema("ringops").from("events").insert({
        promoter_organization_id:promoter.id,
        name,
        event_date:date,
        venue_name:venue,
        status:"planning",
      ...
      setError("興行を作成できませんでした。興行主組織の権限を確認してください。");
    }
  }
  ...
  async function addBout(event:EventItem){
    setError("");
    setBusyId(event.id);
    const rounds=Number(boutRounds);
    const weight=boutWeight?Number(boutWeight):null;
    try{
      if(reviewMode){
        const bout:Bout={id:`bout-${Date.now()}`,eventId:event.id,boxerA:"選手A未定",boxerB:"対戦相手募集中",rounds,weight:weight!=null?`${weight}kg`:"—",status:"募集中"};
        ...
        const {data:created,error:insertError}=await supabase.schema("ringops").from("bouts").insert({
          event_id:event.id,
          bout_date:event.date,
          venue_name:event.venue,
          contract_weight_kg:weight,
          scheduled_rounds:rounds,
          matchmaking_status:"recruiting",
        }).select("id").single();
        if(insertError)throw insertError;
        const bout:Bout={id:created.id,eventId:event.id,boxerA:"選手A未定",boxerB:"対戦相手募集中",rounds,weight:weight!=null?`${weight}kg`:"—",status:"募集中"};
        setEvents(current=>current.map(item=>item.id===event.id?{...item,bouts:[...item.bouts,bout]}:item));
      }
      setBoutDraftEventId("");setBoutWeight("");setBoutRounds("6");
    }catch{
      setError("対戦枠を追加できませんでした。");
    }finally{
      setBusyId("");
    }
  }
  ...
  return <>
    ...
    <div className="divide-y divide-slate-200 border-y-2 border-slate-950 bg-white">
      {events.map(event=><article className="grid gap-5 px-5 py-6 lg:grid-cols-[280px_1fr]" key={event.id}>
        <div>
          <div className="flex items-start justify-between gap-2"><h2 className="text-lg font-black">{event.name}</h2></div>
          <p className="mt-3 text-sm font-black">{event.date}</p>
          <p className="mt-1 text-xs text-slate-500">{event.venue}</p>
          <p className="mt-1 text-xs text-slate-500">{event.promoter}</p>
        ...
        <div>
          ...
          {boutDraftEventId===event.id&&<div className="grid gap-2 border-b border-slate-300 bg-slate-50 p-3 sm:grid-cols-[130px_130px_auto] sm:items-end">
          ...
          {event.bouts.map(bout=><div className="grid gap-3 border-b border-slate-200 py-3 text-sm sm:grid-cols-[1fr_130px_110px] sm:items-center" key={bout.id}>
            <div><b>{bout.boxerA} vs {bout.boxerB}</b><p className="mt-1 text-xs text-slate-500">{bout.weight} / {bout.rounds?`${bout.rounds}R`:"R未定"}</p></div>
            ...
            {bout.status==="募集中"?<Link className="text-xs font-black underline underline-offset-4" href="/open-matches">対戦相手募集へ</Link>:bout.status==="交渉中"?<Link className="text-xs font-black underline underline-offset-4" href="/matchmaking">案件を見る</Link>:<span className="text-xs font-black text-emerald-700">カード決定</span>}
          </div>)}
